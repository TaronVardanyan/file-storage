...
export default function FilesBrowser({title, isFavorites = false, deleteOnly = false}: Props) {
    ...
    const passedType = type === 'all' ? undefined : type;
    const user = useUser();
    const orgId = organization?.id ?? user.user?.id;
    const favorites = useQuery(api.files.getAllFavorites, orgId ? {orgId} : 'skip');
    const files = useQuery(
        api.files.getFiles,
        orgId ? {orgId, type: passedType, query, favorites: isFavorites, deleteOnly} : 'skip',
    );
            ...
            <Tabs defaultValue="grid">
                <div className="flex justify-between items-center">
                    <TabsList className="mb-8">
                        ...
                        <TabsTrigger className="flex gap-2 items-center" value="table"><TableIcon/> Table</TabsTrigger>
                    </TabsList>
                    <div>
